...
def dispatch(base_url, handle, action, params):
    ...
    return func(base_url, handle, params) if func not in (show_profiles, manage_profiles) else func(base_url, handle)
...
def show_profiles(base_url, handle):
    ...
    ui.add_directory(handle, base_url, "Configurações", "settings", "settings.png")
...
def open_settings_route(base_url, handle, params):
...
def manage_profiles(base_url, handle):
...
def show_sailetv(base_url, handle, params):
    ...
    ui.add_directory(handle, base_url, "Busca", "sailetv_search", "search.png", {"profile_id": p["id"]})
    ui.add_directory(handle, base_url, "Favoritos SaileTV", "favorites", "favorites.png", {"profile_id": p["id"], "provider": "xtream"})
    ui.add_directory(handle, base_url, "Sincronizar / Atualizar Conteúdo", "sailetv_index", "update.png", {"profile_id": p["id"]})
    ui.end(handle)
...
def show_sailetv_items(base_url, handle, params):
    ...
    for item in xtream.items(kind, category_id):
        ...
        if kind == "series":
            ...
            ui.add_playable(handle, base_url, title, "play", play_url, artwork if artwork else "vod.png", {"profile_id": p["id"], "provider": "xtream", "media_type": kind, "external_id": item.get("stream_id") or item.get("id") or "", "title": title}, {"title": title, "plot": item.get("plot") or ""})
    ui.end(handle)
...
def show_sailetv_series_info(base_url, handle, params):
    ...
    if isinstance(episodes, dict):
        for season, items in episodes.items():
            for ep in items:
                title = "T%sE%s - %s" % (season, ep.get("episode_num") or ep.get("id") or "", ep.get("title") or "Episódio")
                play_url = xtream.playback_url("episode", ep)
                ui.add_playable(handle, base_url, title, "play", play_url, "series.png", {"profile_id": p["id"], "provider": "xtream", "media_type": "episode", "external_id": ep.get("id") or "", "title": title}, {"title": title, "plot": ep.get("info", {}).get("plot", "") if isinstance(ep.get("info"), dict) else ""})
    ui.end(handle)
...
def show_sailefy(base_url, handle, params):
    ...
    ui.add_directory(handle, base_url, "Minhas playlists", "sailefy_playlists", "playlists.png", {"profile_id": p["id"]})
    ui.add_directory(handle, base_url, "Busca", "sailefy_search", "search.png", {"profile_id": p["id"]})
    ui.add_directory(handle, base_url, "Favoritos SaileFy", "favorites", "favorites.png", {"profile_id": p["id"], "provider": "youtube"})
    ui.end(handle)
...
def sailefy_search(base_url, handle, params):
    ...
    if not p:
        return show_profiles(base_url, handle)
    q = xbmcgui.Dialog().input("Buscar música")
    if not q:
...
def sailefy_playlists(base_url, handle, params):
    notify("Playlists locais: estrutura SQLite pronta para a próxima iteração.")
    ui.end(handle)
