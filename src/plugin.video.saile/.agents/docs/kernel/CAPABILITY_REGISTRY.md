...
Capability Registry defines a Kernel subsystem that coordinates local runtime behavior without owning business rules.
...
- Defines a Kernel-owned manager or registry that coordinates runtime behavior without owning media business rules.
- Must expose contracts and diagnostics while keeping implementation details replaceable.
- Capability ownership includes identifier, version, request model, response model, error model, timeout, cancellation and permissions.
- Capability Registry ownership includes registration, lookup, version negotiation, deterministic selection and health-aware routing metadata.

## Registration Examples

```python
# Example of registering a capability
registry = CapabilityRegistry()
registry.register(
    capability_id="media.search",
    version="1.0.0",
    handler=search_plugin_handler
)

# Example of looking up a capability
handler = registry.lookup("media.search", version="^1.0.0")
```
