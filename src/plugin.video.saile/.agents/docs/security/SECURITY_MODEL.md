...
Security Model defines security controls enforced by the Kernel and its managers.
...
1. **No Hidden Telemetry**: The Kernel and Plugins must NOT collect or transmit any usage analytics, crash reports, or telemetry data to any external server.
2. **Strict Network Isolation**: Only Plugins with explicitly declared `network.*` permissions in their manifest are allowed to make HTTP/TCP requests.
3. **No Direct Sockets**: The `socket` library is strictly forbidden in all scopes to prevent hidden tunnels.
