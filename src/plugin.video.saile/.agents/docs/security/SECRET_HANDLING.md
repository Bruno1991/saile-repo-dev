...
Secret Handling defines security controls enforced by the Kernel and its managers.
...
- Defines Kernel-enforced controls for permissions, resources, secrets and Plugin isolation.
- Must treat third-party data and Plugin output as untrusted until validated.

## Secret Handling Rules

1. **No Hardcoded Secrets**: Plugins and Engines must not contain hardcoded API keys or credentials.
2. **Log Redaction**: The Logging Service MUST automatically intercept and redact any string that matches the pattern of a registered secret or API key before writing to `kodi.log`.
3. **Secure Storage**: Secrets (like OAuth tokens) must be stored using the Kodi settings API with `hidden="true"`, ensuring they are not exported in plain text configurations.
