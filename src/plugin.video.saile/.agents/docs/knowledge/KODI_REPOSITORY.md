...
Kodi Repository captures technical knowledge required to design SAILE without violating architecture.
...
- Captures background knowledge only as it constrains SAILE architecture.
- Must not become a substitute for official SAILE contracts or ADRs.
- Repository ownership includes package metadata, compatibility, checksums, update state and rollback planning without remote SAILE infrastructure.
- Kodi knowledge is used only to build the Presentation Layer and addon host integration; Kodi must not own SAILE business logic.
