...
Plugin SDK defines contract rules for contributors who build against SAILE extension surfaces.
...
All plugins MUST provide a `plugin.json` or `plugin.yaml` manifest that contains:
1. `id`: Unique identifier (e.g., `saile.plugin.tmdb`).
2. `version`: Semantic version string.
3. `capabilities`: A list of capability IDs this plugin implements.
4. `permissions`: Explicitly declared permissions (e.g., `network.http`, `storage.local`).
The Plugin Manager will reject any plugin missing these fields.
