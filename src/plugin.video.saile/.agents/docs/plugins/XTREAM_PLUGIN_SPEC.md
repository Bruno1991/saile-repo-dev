...
Xtream Plugin Spec defines Plugin authoring rules for isolated local modules implementing Capabilities.
...
- Defines a local Plugin integration that implements declared Capabilities under Kernel supervision.
- Must declare permissions, health, manifest metadata, errors, timeouts and response mapping.
- Xtream work is limited to a Provider Plugin that may expose catalog, live TV, VOD, series, authentication and playback-source Capabilities.
- Xtream credentials, URLs and account data must remain local and redacted.
