...
Startup Performance defines performance budgets and measurement practices for local media runtime execution.
...
- Defines measurable local performance budgets for constrained Kodi devices.
- Must protect startup, navigation, playback preparation, lookup latency and memory growth.
- Startup must initialize security and resource boundaries before Plugin execution and must avoid remote blocking work.
