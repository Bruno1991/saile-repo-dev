...
Startup Flow defines how the in-memory Runtime is created, operated and destroyed inside the Kodi addon process.
...
- Defines addon-process lifecycle behavior from bootstrap through shutdown.
- Must keep the Runtime in-process and bounded; it is never a daemon or service.
