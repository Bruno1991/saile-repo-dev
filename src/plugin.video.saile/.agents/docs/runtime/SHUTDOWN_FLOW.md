...
Shutdown Flow defines how the in-memory Runtime is created, operated and destroyed inside the Kodi addon process.
...
- Shutdown must cancel owned work, unload Plugins, flush local state and release resources with bounded waits.
