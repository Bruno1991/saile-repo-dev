...
The SQLite Engineer skill defines how an autonomous or human-assisted engineering contributor acts when working on SAILE.

SQLite Engineer defines an AI engineering role with authority, inputs, outputs and review checklists.

## Responsibilities

The SQLite Engineer is responsible for:
...
The SQLite Engineer may decide local documentation structure and subject-specific review guidance.

The SQLite Engineer must escalate when a change affects Kernel responsibilities, Capability contracts, Plugin permissions, Local-First guarantees, security posture, privacy policy or public SDK contracts.
...
Work performed by the SQLite Engineer is acceptable when it strengthens architectural integrity, is traceable to authoritative documents, is reviewable by maintainers and does not start production addon implementation.
...
- SQLite knowledge applies to local persistence only; Domain objects must remain persistence-agnostic.
