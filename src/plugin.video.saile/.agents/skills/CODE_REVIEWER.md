...
The Code Reviewer skill defines how an autonomous or human-assisted engineering contributor acts when working on SAILE.

Code Reviewer defines an AI engineering role with authority, inputs, outputs and review checklists.
...
The Code Reviewer may decide local documentation structure and subject-specific review guidance.

The Code Reviewer must escalate when a change affects Kernel responsibilities, Capability contracts, Plugin permissions, Local-First guarantees, security posture, privacy policy or public SDK contracts.
...
Work performed by the Code Reviewer is acceptable when it strengthens architectural integrity, is traceable to authoritative documents, is reviewable by maintainers and does not start production addon implementation.
