...
The Playback Engineer skill defines how an autonomous or human-assisted engineering contributor acts when working on SAILE.

Playback Engineer defines an AI engineering role with authority, inputs, outputs and review checklists.

## Responsibilities

The Playback Engineer is responsible for:
...
The Playback Engineer may decide local documentation structure and subject-specific review guidance.

The Playback Engineer must escalate when a change affects Kernel responsibilities, Capability contracts, Plugin permissions, Local-First guarantees, security posture, privacy policy or public SDK contracts.
...
Work performed by the Playback Engineer is acceptable when it strengthens architectural integrity, is traceable to authoritative documents, is reviewable by maintainers and does not start production addon implementation.
...
- Playback ownership includes source resolution policy, ranking, session state, resume, subtitles handoff and player adapter coordination.
